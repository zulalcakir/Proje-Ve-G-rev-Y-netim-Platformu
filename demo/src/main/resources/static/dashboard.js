...
async function istatistikleriGuncelle(userId, token) {
    try {
        ...
        const [resProj, resTask] = await Promise.all([
            fetch(`${BASE_URL}/api/projects/managed-by/${userId}`, { headers }),
            fetch(`${BASE_URL}/api/tasks/user/${userId}`, { headers })
        ]);
...
async function cezalariYukle(userId, token) {
    ...
    try {
        ...
        const resTotal = await fetch(`${BASE_URL}/api/penalties/user/${userId}/total`, { headers });
        ...
        const resList = await fetch(`${BASE_URL}/api/penalties/user/${userId}`, { headers });
...
function markAllNotificationsAsRead() {
    const token = localStorage.getItem('jwtToken');
    const user = JSON.parse(localStorage.getItem('user'));
    fetch(`${BASE_URL}/api/notifications/user/${user.id}/read-all`, {
        method: 'PATCH',
        headers: { 'Authorization': 'Bearer ' + token }
    }).then(res => {
        if(res.ok) {
