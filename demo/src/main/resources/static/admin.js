...
function istatistikleriYukle(token) {
    fetch(`${BASE_URL}/api/admin/stats`, { headers: { 'Authorization': 'Bearer ' + token } })
    .then(handleResponse)
    .then(data => {
        if(data) {
...
function projeleriYukle(token) {
    fetch(`${BASE_URL}/api/projects`, { headers: { 'Authorization': 'Bearer ' + token } })
    .then(handleResponse)
    .then(projects => {
...
function gorevTablosunuYukle(token) {
    fetch(`${BASE_URL}/api/tasks`, { headers: { 'Authorization': 'Bearer ' + token } })
    .then(handleResponse)
    .then(tasks => {
...
function cezaSiralamasiniYukle(token) {
    fetch(`${BASE_URL}/api/penalties`, { headers: { 'Authorization': 'Bearer ' + token } })
    .then(handleResponse)
    .then(penalties => {
        // GRAFİĞİ GÜNCELLE
...
function loglariYukle(token) {
    ...
    fetch(`${BASE_URL}/api/logs`, { headers: { 'Authorization': 'Bearer ' + token } })
    .then(handleResponse)
    .then(logs => {
        if(!logs) return;
...
function populatePrioritySelect(elementId) {
    ...
    if(!select) return;
    const token = localStorage.getItem('jwtToken');
    fetch(`${BASE_URL}/api/priorities`, { headers: { 'Authorization': 'Bearer ' + token } })
...
function populateDepartments() {
    ...
    if(!select) return;
    const token = localStorage.getItem('jwtToken');
    fetch(`${BASE_URL}/api/departments`, { headers: { 'Authorization': 'Bearer ' + token } })
...
function populateCategories() {
    ...
    if(!select) return;
    const token = localStorage.getItem('jwtToken');
    fetch(`${BASE_URL}/api/categories`, { headers: { 'Authorization': 'Bearer ' + token } })
...
function populateTags() {
    ...
    if(!select) return;
    const token = localStorage.getItem('jwtToken');
    fetch(`${BASE_URL}/api/tags`, { headers: { 'Authorization': 'Bearer ' + token } })
...
function dosyalariGetir(taskId, token) {
    ...
    fetch(`${BASE_URL}/api/attachments/task/${taskId}`, { headers: { 'Authorization': 'Bearer ' + token } })
    .then(res => res.json())
    .then(attachments => {
        container.innerHTML = '';
...
function dosyaIndir(attachmentId, fileName) {
    ...
    fetch(`${BASE_URL}/api/attachments/download/${attachmentId}`, { headers: { 'Authorization': 'Bearer ' + token } })
    .then(response => {
...
function kullaniciSil(id) {
    ...
    const token = localStorage.getItem('jwtToken');
    fetch(`${BASE_URL}/api/users/${id}`, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } })
...
function deleteProject(id) {
    ...
    const token = localStorage.getItem('jwtToken');
    fetch(`${BASE_URL}/api/projects/${id}`, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } })
    .then(res => { if(res.ok) verileriTazele(token); });
}
