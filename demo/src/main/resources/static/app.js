...
// --- YENİ EKLENDİ: GENEL VERİ ÇEKME YARDIMCISI ---
// Dashboard veya Admin ekranında görevleri çekerken bu fonksiyonu kullanmalısın
function fetchSecureData(endpoint) {
    const token = localStorage.getItem('jwtToken');
    return fetch(`${BASE_URL}${endpoint}`, {
        method: 'GET',
        headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}` // KRİTİK DÜZELTME
        }
    }).then(res => {
        if (res.status === 401 || res.status === 403) {
            window.location.href = 'index.html'; // Yetkisizse girişe at
