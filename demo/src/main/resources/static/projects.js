/**
 * P-PLATFORM Projeler Sayfası Mantığı
 * Bu dosya kullanıcının yönettiği ve dahil olduğu projeleri AWS üzerinden çeker.
 */
...
async function projelerimiYukle(userId, token) {
    ...
    try {
        // A) Yönettiğim Projeleri Çek (AWS üzerinden)
        const resProj = await fetch(`${BASE_URL}/api/projects/managed-by/${userId}`, { headers });
        const yonettigimProjeler = resProj.ok ? await resProj.json() : [];

        // B) Bana Atanan Görevleri Çek (AWS üzerinden)
        const resTask = await fetch(`${BASE_URL}/api/tasks/user/${userId}`, { headers });
...
async function bildirimSayisiniYukle(userId, token) {
    ...
    try {
        const res = await fetch(`${BASE_URL}/api/notifications/user/${userId}/unread-count`, {
